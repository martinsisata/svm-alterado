...
---Testando repartição de data
SELECT EXTRACT(YEAR FROM dataVenda) AS Ano,
EXTRACT(MONTH FROM dataVenda) AS Mes,
EXTRACT(DAY FROM dataVenda) AS Dia
FROM venda
...
EXTRACT(MONTH FROM dataVenda) AS mes, EXTRACT(DAY FROM dataVenda) AS dia from venda, medicamento, funcionario
	where venda.idMedicamento = medicamento.idMedicamento and venda.idFuncionario=funcionario.idFuncionario;
---trazer todas as vendas de uma determinada data
select * from vendasFiltradas
where mes = 2 and dia = 7 and ano = 2014;
